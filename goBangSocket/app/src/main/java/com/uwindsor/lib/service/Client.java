...
import com.uwindsor.lib.util.NetworkUtil;
...
public class Client {
	...
	private String IP="10.243.16.167";
	private int PORT=1234;
	private int PORT2=4321;
...
Log.v(TAG,"Rec :---------");
							...
							String str="";
							int count=0;
						while((read=inStream.read(bodyBytes))>=0)
							{
								if(read<length){
									str=str+new String(bodyBytes, 0, read);
									String tempstr =str;
									str="";
									respListener.onSocketResponse(tempstr);
								}
								else{
									str=str+new String(bodyBytes);
								}
							}
